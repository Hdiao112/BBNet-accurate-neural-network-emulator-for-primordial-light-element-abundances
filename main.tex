...
Cosmological parameter estimation typically demands a trade-off between computational speed and theoretical precision. To accelerate Monte Carlo sampling, standard pipelines often rely on reduced nuclear networks or simplified ordinary differential equation (ODE) integrators. While computationally efficient, these approximations introduce systematic biases that can propagate into the final inference.

In this section, we argue that the \verb|BBNet| emulator renders this compromise obsolete. By benchmarking \verb|BBNet| against the reduced-network modes of \verb|PArthENoPE| and the simplified integration schemes of \verb|AlterBBN|, we show that the emulator retains the accuracy of the complete reaction network while matching the throughput of the simplified methods.
Consequently, \verb|BBNet| serves as a comprehensive replacement for these approximate first-principles calculations in high-precision analyses.

The evaluation proceeds in two parts: Sec .~\ref {sec:mcmc} quantifies the sampling throughput within a realistic MCMC framework, and Sec .~\ref{accuracy_vs} analyzes the residual distributions to demonstrate the elimination of systematic biases characteristic of reduced networks.
...
To quantify the computational acceleration provided by \verb|BBNet| within a realistic cosmological parameter estimation pipeline, we conducted benchmark tests under Markov Chain Monte Carlo (MCMC) conditions. Benchmarks were performed using a single MCMC chain of $1000$ steps for each solver, allowing for a direct assessment of wall-clock time and sampling throughput.
...
We benchmark the emulator against two standard BBN codes, selecting configurations that represent the traditional spectrum of the speed-accuracy trade-off. For \verb|PArthENoPE|, we compare the standard high-accuracy mode (complete network, 26 nuclides via 100 ODEs) against the simplified approximation (small network, 9 nuclides via 40 ODEs~\cite{2008CoPhC.178..956P}).
The latter represents a typical strategy to accelerate inference at the cost of theoretical completeness. For \verb|AlterBBN|, we examine the high-precision RK2\_halfstep mode (\verb|failsafe=7|), which enforces strict consistency checks, versus the standard RK2 mode (\verb|failsafe=3|), a faster scheme lacking half-step validation~\cite{2018arXiv180611095A}. The standard RK2 mode serves as a baseline for the fastest available direct numerical integration.
...
The results, summarized in Table~\ref{tab:mcmc_speed_comparison}, demonstrate that \verb|BBNet| provides substantial gains in computational efficiency. Relative to the complete \verb|PArthENoPE| network, the emulator achieves a speedup factor of $\sim 7.4\times10^{3}$. More significantly, it outperforms the simplified small-network approximation by a factor of $\sim 360$. This finding renders the small-network approximation obsolete: \verb|BBNet| is not only theoretically accurate (as it reproduces the full network) but also computationally faster than the reduced code.
...
Similarly, for the \verb|AlterBBN| group, \verb|BBNet| accelerates inference by $\sim 2.7\times10^{3}$ relative to the rigorous RK2\_halfstep integrator and exceeds the speed of the standard RK2 mode by a factor of $\sim 75$. The moderately higher overhead observed in the \verb|AlterBBN| emulator, compared to the \verb|PArthENoPE| counterpart, arises from the internal expert mode mechanism used to handle adaptive integration boundaries (see Appendix~\ref{app: experts}). This overhead, however, does not compromise the overall efficiency, as the total inference time for 1000 steps is reduced from several hours to seconds.

\subsection{Residual distributions in emulator outputs}
\label{accuracy_vs}

While the previous subsection established the computational efficiency of \verb|BBNet|, its practical utility for precision cosmology depends critically on prediction accuracy. Simplified numerical schemes achieve acceleration by sacrificing theoretical completeness. This trade-off typically manifests as consistent directional biases in the predicted abundances, which act as systematic errors that can propagate into the posterior distributions of cosmological parameters. In contrast, a robust emulator must reproduce the ground truth of the complete physics network without introducing such serious shifts.

We quantify the accuracy of \verb|BBNet| by analyzing the distribution of relative errors with respect to the high-precision baselines (the complete network for \verb|PArthENoPE| and the RK2\_halfstep mode for \verb|AlterBBN|).
The statistical metrics, including the mean percentage error (MPE) as a measure of bias and the root-mean-square percentage error (RMSPE) as a measure of dispersion, are summarized in Table~\ref{tab:error_summary_combined}.
...
\begin{figure*}[t]
  ...
  \subfloat[\label{fig:error_dh}]{%
    ...
    \caption{\raggedright
    Distributions of relative percentage errors for $Y_\mathrm{P}$ (left panel) and $\mathrm{D/H}$ (right panel) evaluated over the AlterBBN test set.
    Errors are computed with respect to the high-accuracy reference solution ($\mathbf{y}_{\mathrm{ref}}$) obtained using the RK2 integrator with half-step validation (RK2\_halfstep).
    Blue histograms represent the residuals of the standard RK2 mode ($\mathbf{y}_{\mathrm{RK2}}$), illustrating the systematic bias introduced by the simplified integration scheme.
    Orange histograms show the residuals of the BBNet emulator ($\mathbf{y}_{\mathrm{BBNet}}$).
    The relative error is defined as $\delta(\%) = 100 \times (y_{\mathrm{method}} - y_{\mathrm{ref}}) / y_{\mathrm{ref}}$.
    Dashed vertical lines indicate the mean signed error (MPE) for each method.
    Note that the horizontal axes are zoomed to $\pm 0.05\%$ for $Y_\mathrm{P}$ and $\pm 0.5\%$ for $\mathrm{D/H}$ to resolve the fine structure of the distributions.}
    \label{fig:error_comparison}
\end{figure*}

For the \verb|PArthENoPE| dataset (see the upper block of Table~\ref{tab:error_summary_combined}), \verb|BBNet| demonstrates a distinct advantage in bias elimination. The small network approximation introduces a systematic offset of $\sim 0.08\%$ in $\mathrm{D/H}$, a deviation that is consistent across the parameter space. \verb|BBNet| reduces this mean error to a negligible level ($0.0013\%$).
Although the emulator exhibits a marginally higher RMSPE for $Y_\mathrm{P}$ compared to the small network ($0.0158\%$ versus $0.0088\%$), this dispersion represents symmetric, zero-mean stochastic noise. In the context of Bayesian inference, such unbiased scatter is preferable to the rigid systematic shifts inherent in the reduced-network approximation, as it avoids shifting the centroids of posterior probability distributions.

The advantage of the emulator is further amplified in the comparison with \verb|AlterBBN| (lower block of Table~\ref{tab:error_summary_combined} and Fig.~\ref{fig:error_comparison}). The standard RK2 solver exhibits a significant systematic drift, underestimating $\mathrm{D/H}$ by $\sim 0.16\%$ with an RMSPE of $\sim 0.20\%$. \verb|BBNet| effectively corrects this behavior. As illustrated in Fig.~\ref{fig:error_comparison}, the residual distribution of the emulator (orange histograms) is centered at zero, faithfully reproducing the reference RK2\_halfstep solution without the systematic skew observed in the fast integrator (blue histograms).

Quantitatively, \verb|BBNet| reduces the RMSPE in $\mathrm{D/H}$ by a factor of $2.5$ relative to the standard RK2 solver, while simultaneously suppressing the mean bias to below $10^{-3}\%$. These results confirm that \verb|BBNet| supersedes the simplified numerical schemes. By delivering the statistical accuracy of full-network calculations with negligible systematic bias, the emulator ensures that cosmological constraints are limited only by observational precision rather than by the computational shortcuts of the theoretical solver.


\section{Conclusion}
\label{sec:conclusion}

We have presented \verb|BBNet|, a deep learning emulator designed to accelerate Big Bang Nucleosynthesis (BBN) calculations without compromising physical accuracy.
The network employs a residual multi-head attention architecture to capture the non-linear dependencies of primordial abundances on cosmological parameters.
We trained the model using Latin-hypercube samples derived from two independent BBN codes, \verb|PArthENoPE| and \verb|AlterBBN|, ensuring consistent nuclear physics inputs across both datasets.
The training domain covers the standard $\Lambda$CDM parameters as well as extensions involving dark radiation and stiff-fluid cosmologies.

Our evaluations demonstrate that \verb|BBNet| effectively resolves the traditional conflict between computational speed and theoretical precision.
Across both code bases, the emulator reproduces the benchmark predictions of the complete reaction networks with negligible mean bias and sub-percent dispersion.
Crucially, \verb|BBNet| eliminates the systematic offsets inherent in reduced-network approximations (such as the \verb|PArthENoPE| small network) and simplified integration schemes. In terms of efficiency, the emulator achieves inference speedups of $\mathcal{O}(10^{3})$ relative to the fastest reduced networks and up to $\mathcal{O}(10^{4})$ compared to high-accuracy ODE solvers.
These gains allow for the replacement of approximate methods with high-fidelity emulation at minimal computational cost.

This combination of accuracy and throughput removes the primary bottleneck in large-scale cosmological parameter estimation.
\verb|BBNet| serves as a robust, bias-resistant module for forward modeling within Bayesian pipelines, enabling rigorous exploration of high-dimensional parameter spaces.
Furthermore, the framework remains flexible: unlike static interpolation grids, the network can be efficiently retrained to accommodate updated nuclear reaction rates or expanded prior ranges.

Future work will extend the emulator to predict the primordial abundances of Helium-3 and Lithium-7. This will facilitate a comprehensive investigation of the ``cosmological lithium problem,'' allowing for dedicated MCMC studies that jointly constrain reaction-rate uncertainties and beyond-Standard-Model effects.
All source code, pretrained models, and datasets used in this work are publicly available at \url{https://github.com/Hdiao112/BBNet.git}.
...
\appendix
\section{Hierarchical Expert Mode for Adaptive Refinement}
\label{app:experts}

To address the wide dynamic range and complex distribution of the \verb|AlterBBN| dataset, we implement a two-stage hierarchical inference strategy. The architecture consists of a global \emph{base} model, trained on the complete dataset, augmented by two domain-specialized \emph{expert} models trained on filtered subsets of the parameter space. All three networks share an identical backbone architecture (\verb|ResMLPWithAttn|).
Each model checkpoint is stored alongside its corresponding normalization statistics (scalers) to ensure that the input-output transformations remain strictly reversible and consistent with the specific training distribution of each expert.

During the inference phase, the framework employs a hard-gating mechanism (activated via the \verb|--exp| flag). The base model first generates a preliminary prediction, $\widehat{\mathrm{D/H}}_{\text{base}}$. This value acts as a decision variable to route the input to the appropriate specialist network or to accept the base prediction, according to the following rule:
\begin{equation}
  \widehat{{\mathrm{D/H}}_{\text{out}}} = 
  \begin{cases} 
    \text{Expert 2} & \text{if } \widehat{\mathrm{D/H}}_{\text{base}} \in [10^{-7},\,10^{-5}), \\
    \text{Expert 1} & \text{if } \widehat{\mathrm{D/H}}_{\text{base}} \in [10^{-5},\,10^{-3}], 
  \end{cases}
\end{equation}
For routed samples, the raw inputs are renormalized using the target expert's specific statistics before being processed by the expert network.
This band-specific renormalization is essential because the statistical moments of the filtered subsets differ significantly from the global distribution, and using global scalers for local experts would degrade precision.

We quantify the computational overhead of this approach in terms of per-sample wall-clock time and theoretical floating-point operations (FLOPs).
In the \verb|AlterBBN| test run ($N=3000$), the routing fraction is close to unity ($f\!\approx\!1$), meaning nearly all samples trigger a second forward pass. The resulting resource consumption is summarized in Table~\ref{tab:exp_cost_numeric}. Since the expert mechanism requires loading up to three distinct sets of weights, the memory footprint increases by factor of $\sim3$ compared to the base-only mode, although this can be mitigated to $\sim2\times$ via lazy loading.

\begin{table}[ht]
  \caption{\raggedright
    Comparison of computational resources and latency between the \emph{base-only} mode and the \emph{expert} mode.
    Wall times are measured per sample over repeated trials.
    FLOPs are calculated for a single inference pass given the specific backbone configuration.
    The memory column reports the FP32 model weight footprint plus the storage required for the normalization scalers.}
  \label{tab:exp_cost_numeric}
  \begin{ruledtabular}
  \begin{tabular}{lccp{0.3\linewidth}}
    \textbf{Inference Mode} & \textbf{Latency ($\mu$s)} & \textbf{FLOPs} & \textbf{Memory} \\
    \hline
    Base only
      & 36.5--56.2
      & $4.03\times10^{8}$
